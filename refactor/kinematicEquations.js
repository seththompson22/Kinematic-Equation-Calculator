...
function notFinalVelocity(displacement, initialVelocity, time, acceleration, target) {
    ...
    // if time is the unknown target
        ...
        var underSQRT = Math.sqrt(Math.pow(initialVelocity, 2) + (2 * acceleration * displacement));
        var time1 = ((-1 * initialVelocity) + underSQRT) / acceleration;
        var time2 = ((-1 * initialVelocity) - underSQRT) / acceleration;
        if ((time1 >= 0) && (time2 >= 0)) {
...
function notInitialVelocity(displacement, finalVelocity, time, acceleration, target) {
    ...
    // if time is the unknown target
        ...
        var underSQRT = Math.sqrt(Math.pow(finalVelocity, 2) + (2 * acceleration * displacement));
        var time1 = ((-1 * finalVelocity) + underSQRT) / (-1 * acceleration);
        var time2 = ((-1 * finalVelocity) - underSQRT) / (-1 * acceleration);
        if ((time1 >= 0) && (time2 >= 0)) {
            System.out.println("Time: " + time1);
            return time2;
        }
